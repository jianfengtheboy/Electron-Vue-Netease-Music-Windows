 ...
 * @Date: 2020-06-14 18:22:14
 * @LastEditTime: 2020-06-14 18:30:10
 * @Description: player
-->
<template>
  <div class="player-wrapper">
    <div class="video-player" @dblclick="toggleFullscreen">
      <transition name="topbar">
        <div class="topbar-wrapper" :class="{ 'topbar-with-fullscreen' : isFullScreen, 'hide'  : isFixed }"
          ...
          <div class="topbar">
            ...
            <div class="title js-video-title" v-if="videoData.name">
              <div class="inner js-video-title-inner">{{ videoData.name }} {{ videoData.desc }}</div>
            </div>
            <div class="actions">
              <a-icon type="pushpin" @click="toggleAlwaysOnTop" :theme="isAlwaysOnTop ? 'filled' : 'outlined'" />
              <a-icon type="minus" @click="minimize" />
              <a-icon :type="isFullScreen ? 'switcher' : 'border'" @click="toggleFullscreen" />
              <a-icon :type="isMaximized ? 'fullscreen-exit' : 'fullscreen'" @click="maximize" />
      ...
      <transition name="controls">
        <div class="controls-wrapper"
             :class="{ 'controls-with-fullscreen' : isFullScreen, 'hide'  : isFixed }"
             v-show="!isFullScreen || isMoving">
          <div class="video-controls">
            ...
            <div class="video-operation">
              <div class="left">
                ...
                <i>|</i>
                <span class="time">{{videoData.duration / 1000 | duration}}</span>
              </div>
              <div class="right">
                <div class="voice">
      ...
      <div class="mask-with-fixed" v-show="isFixed">
        <a-icon type="arrows-alt" title="标准模式" @click="restoreWin" class="icon-right-bottom" />
        <a-icon :type="playIcon" @click="togglePlaying" class="icon-center" />
        <a-icon type="close" title="关闭" @click="close" class="icon-right-top" />
      </div>
      <a-icon :type="showMoerIcon" class="switch" @click="toggleShowMore" v-show="!isFixed" />
    </div>
      ...
      <a-tabs default-active-key="1" @change="handleTabChange">
        <a-tab-pane key="video" tab="视频">
          <div class="mv-info">
            ...
            <div style="display: flex;" v-if="videoType === 'video' && videoData.videoGroup.length">
              <span style="minWidth: 60px;marginRight:3px;flex: 0 0 40px;color: #777;">标签：</span>
              <a-breadcrumb>
                <a-breadcrumb-item v-for="tag in videoData.videoGroup" :key="tag.id">
                  <router-link :to="`/video?groupId=${tag.id}`" class="tag">{{ tag.name }}</router-link>
                </a-breadcrumb-item>
              </a-breadcrumb>
            </div>
            <div class="publishTime" v-if="videoData.publishTime">发布时间：{{ videoData.publishTime | toDate }}</div>
            <div class="playCount">播放次数：{{ videoData.playCount | toWan }}</div>
          ...
          <div class="simi-mv">
            ...
            <div class="list">
              <template v-if="videoType === 'video'">
                ...
                >
                  <img class="avatar" v-lazy="`${item.avatar}?param=560y320`" :key="item.id" alt="">
                  <div class="info">
        ...
        <a-tab-pane key="comment" tab="评论">
          <template v-if="commentData">
            ...
            <a-pagination size="small" :total="commentData.total" @change="handlePageChange" />
          </template>
        </a-tab-pane>
      </a-tabs>
    </div>
...
import { getMVInfo, getSimiMV } from '@/api/mv'
import { getVideoUrl, getVideoDetail, getRelatedVideo } from '@/api/video'
...
export default {
  ...
  components: {
    ProgressBar, Comment, Loading, Artists, ZIcon
  },
  computed: {
    ...mapState('Video', [
      'videoId',
  ...
  watch: {
    ...
    isFixed (newVal) {
      if (newVal) {
        ...
        win.setBounds({ x: 0, y: 0, width: WIN_MIN_WIDTH, height: WIN_MIN_WIDTH * (9 / 16) })
  ...
  mounted () {
    ...
    this.$refs.videoIntro.addEventListener('transitionend', debounce(this.handleResize, 300))
...
.mask-with-fixed {
  ...
  &:hover {
    ...
    .icon-center, .icon-right-bottom, .icon-right-top {
      display: block;
    }
  }
  .icon-center {
