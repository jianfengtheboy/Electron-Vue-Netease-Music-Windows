 ...
 * @LastEditTime: 2020-06-14 18:57:49
 * @Description: 🔍搜索--歌词
 -->
<template>
  <a-spin :spinning="spinning">
    <track-list :tracks="songs" @dblclick="play" @download="download">
      <div slot="lyric" slot-scope="{ row }" class="lyrics-wrapper">
        <div class="lyrics">
          <div
            v-for="(text, index) in row.lyrics.showAll ? row.lyrics.lines : row.lyrics.lines.slice(0, 4)"
            :key="index"
            class="line"
          >
            {{ text }}
        ...
        <div class="actions">
          <a-button
            size="small"
            @click.stop="toggleSpread(row.lyrics)"
            @dblclick.stop
            style="margin-right: 3px"
          >
            {{ row.lyrics.showAll ? '收起歌词' : '展开歌词' }}
          </a-button>
